...
public class MeleeAttackAbility extends Ability {
    ...
    public MeleeAttackAbility(Vec enemyVec) {
        this.enemyVec = enemyVec;
    }

    public MeleeAttackAbility(ELineType l,int i) {
        this.enemyVec = new Vec(l,i);
    }

    public MeleeAttackAbility(ELineType l) {
        this.enemyVec = new Vec(l,0);
    }

    public MeleeAttackAbility() {
        this.enemyVec = new Vec(ELineType.FRONT,0);
    }
    ...
    public boolean canPerform(MatchState state, CardInstance card) {
        ...
        if (!Conditions.IS_FRONTLINE(card)){
            return false;
        }

        // Is enemy protected from frontline?
        Pos enemyPos = computeEnemyPos(card);
        if(enemyPos.isBack()){
            if(!Conditions.IS_FREE_POSITION(state,enemyPos.front())){
                return false;
            }
        }
